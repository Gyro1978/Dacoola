...
const synth = window.speechSynthesis; // Browser's speech engine
let currentUtterance = null;       // The speech object being spoken
let currentPlayingButton = null; // Button associated with the active speech
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadNavbar().then(() => {
        ...
        initializePageContent(); // Determine page type and load content
    }).catch(error => {
...
function timeAgo(isoDateString) {
    // Keep existing timeAgo function... (no changes needed)
...
function updateTimestamps() {
    // Keep existing updateTimestamps function... (no changes needed)
...
async function loadNavbar() {
    // Keep existing loadNavbar function... (no changes needed)
...
async function loadHomepageData() {
    // Keep existing loadHomepageData function... (no changes needed)
...
function renderBreakingNews(articles) {
    // Keep existing renderBreakingNews function... (no changes needed)
...
function renderLatestNews(articles) {
    // Keep existing renderLatestNews function... (no changes needed)
...
function renderTopics() {
    // Keep existing renderTopics function... (no changes needed)
...
function renderTrendingNews(articles) {
    // Keep existing renderTrendingNews function... (no changes needed)
...
async function loadGenericPageData() {
    // Keep existing loadGenericPageData function... (no changes needed)
...
function calculateSearchScore(article, searchTokens) {
    // Keep existing calculateSearchScore function... (no changes needed)
...
function renderArticleCardList(container, articles, emptyMessage) {
    // Keep most of renderArticleCardList...
    ...
    articles.forEach(article => {
        ...
        card.className = container.closest('.sidebar') ? 'article-card sidebar-card' : 'article-card'; // Adapt style for sidebars

        const linkPath = `/${article.link}`; // Use absolute path from web root

        const topic = article.topic || "News";
        const isBreaking = article.is_breaking || false;
        let showBreakingLabel = false;
        // Only show breaking label if recent (e.g., within 6 hours)
        if (isBreaking && article.published_iso) {
            try {
                if ((new Date() - new Date(article.published_iso)) / (1000 * 60 * 60) <= 6) {
                    showBreakingLabel = true;
                }
            } catch {}
        }

        // *** Simplified Audio Button: Always use browser TTS icon/title ***
    ...
    updateTimestamps(); // Update timestamps for newly added cards
}


// --- Sidebar Data Loading & Rendering (For Article Pages) ---
async function loadSidebarData() {
    // Keep existing loadSidebarData function... (no changes needed)
...
function setupSearch() {
    // Keep existing setupSearch function... (no changes needed)
...
// --- Browser TTS Playback Logic (Simplified) ---
function setupBrowserTTSListeners() {
    ...
    document.body.removeEventListener('click', handleTTSDelegatedClick); // Remove previous listener if any
    document.body.addEventListener('click', handleTTSDelegatedClick);

    // Reset global button state if needed
    const globalButton = document.getElementById('global-tts-player-button');
    if (globalButton && currentPlayingButton !== globalButton) {
        resetTTSButtonState(globalButton);
    } else if (globalButton && synth.paused && currentPlayingButton === globalButton) {
        // If page reloaded while paused on global button
        globalButton.innerHTML = '<i class="fas fa-play"></i>'; // Show play icon
    }

    // Clean up any stray 'playing' states on card buttons
    document.querySelectorAll('.listen-button.playing').forEach(button => {
        if (button !== currentPlayingButton) { resetTTSButtonState(button); }
    });

    // Ensure speech is cancelled if user navigates away
...
function cancelSpeech() {
    ...
    if (currentPlayingButton) {
        resetTTSButtonState(currentPlayingButton); // Reset the button visually
    }
...
function resetTTSButtonState(button) {
    if (button) {
        ...
        button.innerHTML = '<i class="fas fa-headphones"></i>'; // Default icon
        button.disabled = false;
        // If this was the button being reset, clear the global state tracking
...
function handleTTSDelegatedClick(event) {
    // Find the closest relevant button that was clicked
    const button = event.target.closest('.listen-button, #global-tts-player-button');
    if (!button || !synth) return; // Ignore if not a TTS button or synth not supported
    ...
    if (isGlobalButton) {
        ...
        console.log(`Global TTS button clicked. Text length: ${textToSpeak.length}`);
    } else { // It's a card button
        const card = button.closest('.article-card');
        const titleElement = card?.querySelector('h3');
        // *** Let's speak the TITLE from the card button ***
        textToSpeak = titleElement ? (titleElement.textContent || titleElement.innerText).trim() : '';
        console.log(`Card TTS button clicked. Speaking title: "${textToSpeak}"`);
    }
    ...
    if (currentPlayingButton === button) {
        ...
        if (synth.paused) {
            ...
            synth.pause(); // Pause instead of cancel
            button.innerHTML = '<i class="fas fa-play"></i>';
        } else {
            // If synth stopped unexpectedly but button state wasn't reset
             console.log("Synth not active, resetting button.");
             resetTTSButtonState(button);
        }
    } else {
        // Clicked a new button (or the first time)
        cancelSpeech(); // Stop any previous speech first

        if (!textToSpeak) {
            console.warn("No text content found to speak.");
            alert("No content available to speak for this item.");
            resetTTSButtonState(button); // Reset immediately if no text
            return;
        }

        // Use the speakText function to handle new utterance
...
function speakText(text, button) {
    if (!synth || !text || !button) {
        console.error("speakText called with invalid args:", { synth, text, button });
        if(button) resetTTSButtonState(button); // Reset if button exists
        return;
    }
    ...
    // Truncate very long text for browser TTS stability
    const MAX_TTS_CHARS = 3000; // Adjust as needed
    if (text.length > MAX_TTS_CHARS) {
        console.warn(`Text too long for browser TTS (${text.length} chars), truncating.`);
    ...
    currentUtterance.onerror = (e) => {
        ...
        // Avoid alerting for common interruptions
        if (e.error && e.error !== 'interrupted' && e.error !== 'canceled') {
            alert(`Speech error: ${e.error}`);
        } else {
            console.log("Speech error/interruption:", e.error || "Unknown");
        }
        resetTTSButtonState(button); // Reset button on error
    };

    currentUtterance.onend = () => {
        console.log("Browser Speech finished.");
        // Only reset the button if it's still the one associated with this utterance
        if (currentPlayingButton === button) {
            resetTTSButtonState(button);
        }
    };
    ...
    currentUtterance.onstart = () => {
        ...
        button.innerHTML = '<i class="fas fa-pause"></i>';
        button.disabled = false;
        currentPlayingButton = button; // Track the button that started speech
    };
