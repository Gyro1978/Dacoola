...
function timeAgo(isoDateString) {
    // (Keep function as before - unchanged)
...
function updateTimestamps() {
    // console.log("Updating timestamps..."); // Less verbose
...
async function loadNavbar() {
    // (Keep function as before - unchanged, using absolute path)
    const navbarPlaceholder = document.getElementById('navbar-placeholder'); if (!navbarPlaceholder) { console.error("Navbar placeholder missing."); return Promise.reject("Navbar placeholder missing"); } const navbarPath = '/templates/navbar.html'; try { console.log(`Fetching navbar: ${navbarPath}`); const response = await fetch(navbarPath); if (!response.ok) { throw new Error(`Failed fetch ${navbarPath}: ${response.status}`); } const navbarHtml = await response.text(); navbarPlaceholder.innerHTML = navbarHtml; console.log("Navbar loaded."); return Promise.resolve(); } catch (error) { console.error('Error loading navbar:', error); navbarPlaceholder.innerHTML = '<p style="color: red; text-align: center;">Err Nav</p>'; return Promise.reject(error); }
}
...
function renderTrendingNews(articles) { // Receives articles from homepage load (site_data.json)
    ...
    // Sort articles by the pre-calculated trend_score (highest first)
    const sortedByTrend = articles
        .slice() // Create a copy to avoid modifying the original array
        .sort((a, b) => (b.trend_score || 0) - (a.trend_score || 0));

    // Take the top 4-5 based on score
    const articlesToShow = sortedByTrend.slice(0, 4); // Show top 4 trending

    if (articlesToShow.length === 0) {
        // This case should be rare if sorting all available articles
    ...
    articlesToShow.forEach(article => {
        ...
        // Link should be absolute from root
        const linkPath = `/public/${article.link}`;
        li.innerHTML = `
            <a href="${linkPath}" class="sidebar-item-link">
                ...
                <div class="sidebar-item-content">
                    ...
                    <!-- Optional: Display score for debugging -->
                    <!-- <span style="font-size: 0.7em; color: grey;">Score: ${article.trend_score || 0}</span> -->
                </div>
            </a>
        `;
        ul.appendChild(li);
...
function calculateSearchScore(article, searchTokens) {
    ...
    // Combine all searchable text fields into one string for simpler matching
    const combinedText = `${title} ${topic} ${tags.join(' ')} ${summary}`;
    const combinedTokens = combinedText.split(/[\s\W]+/).filter(Boolean); // Tokenize combined text

    const queryPhrase = searchTokens.join(' '); // Reconstruct query phrase

    // Score based on individual token presence
    for (const token of searchTokens) {
        if (!token) continue;
        if (title.includes(token)) score += 10; // High weight for title contains token
        if (topic.includes(token)) score += 5;
        if (tags.some(tag => tag.includes(token))) score += 3;
        if (summary.includes(token)) score += 1;
    }

    // Bonus for matching the full query phrase (better for "chat gpt")
    ...
    // Bonus for matching ALL tokens individually (higher relevance)
...
function renderArticleCardList(container, articles, emptyMessage) {
    if (!container) {
        console.error("Target container not found for article list.");
        return;
    }
    container.innerHTML = ''; // Clear existing

    // *** Log the number of articles RECEIVED by the function ***
    ...
    let renderCount = 0; // Counter for rendered cards

    articles.forEach(article => {
        if (!article || !article.id || !article.title) {
             console.warn("Skipping invalid article data:", article);
             return; // Skip this iteration if data is bad
        }
        renderCount++; // Increment counter

        const card = document.createElement('article');
        card.className = container.closest('.sidebar') ? 'article-card sidebar-card' : 'article-card';
        const linkPath = isArticlePage ? `../${article.link}` : `/public/${article.link}`;
    ...
    // *** Log the final count of cards actually added to the DOM ***
    console.log(`renderArticleCardList actually rendered ${renderCount} cards.`);

    setupBrowserTTS(); // Re-setup listeners for new buttons
    updateTimestamps();
}
...
function setupBrowserTTS() {
    ...
    // Reset global button if it exists on this page
    ...
    // Interval check (optional, use with caution)
    // setInterval(() => { if (currentPlayingButton && !synth.speaking && currentUtterance) { console.log("Speech finished unexpectedly (interval), resetting."); resetTTSButtonState(currentPlayingButton); currentUtterance = null; currentPlayingButton = null; } }, 500);
}
...
function handleTTSDelegatedClick(event) {
    ...
    let textToSpeak = ''; let articleId = button.getAttribute('data-article-id'); // Get ID for context

    if (button.id === 'global-tts-player-button') { // Logic for global button
    ...
    if (currentPlayingButton === button && synth.speaking) {
        ...
        synth.cancel(); // onend listener should reset state
    } else {
        ...
        setTimeout(() => speakText(textToSpeak, button), 150); // Delay after cancel
    }
}
...
function speakText(text, button) {
    ...
    currentUtterance.onerror = (e) => {
        ...
        // *** MODIFIED: Don't alert if interrupted ***
    ...
    currentUtterance.onend = () => {
        ...
        // Check if it was the button currently marked as playing before resetting
    ...
    if (currentPlayingButton && currentPlayingButton !== button) { resetTTSButtonState(currentPlayingButton); } // Reset previous if different
