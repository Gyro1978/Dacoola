...
function renderArticleCardList(container, articles, emptyMessage) {
    ...
    setupBrowserTTSListeners(); // Changed name slightly
    updateTimestamps();
}
...
// --- Browser TTS Playback Logic (Simplified & Corrected Pause/Resume) ---
function setupBrowserTTSListeners() { // Renamed slightly for clarity
    if (!synth) {
        console.warn("Browser speech synthesis not supported.");
        // Optionally hide all TTS buttons if not supported
    ...
    // Use event delegation on the body for all TTS buttons
    document.body.removeEventListener('click', handleTTSDelegatedClick); // Prevent multiple listeners
    document.body.addEventListener('click', handleTTSDelegatedClick);

    // Reset global button state on setup if needed
    ...
    if (globalButton && currentPlayingButton !== globalButton) {
        ...
        // Restore play icon if paused during reload
        globalButton.innerHTML = '<i class="fas fa-play"></i>';
    }
...
function cancelSpeech() {
    ...
    if (synth.speaking || synth.pending) {
        console.log("Cancelling/clearing any active speech.");
...
function resetTTSButtonState(button) {
    if (button) {
        button.classList.remove('playing', 'loading'); // Remove status classes
        button.innerHTML = '<i class="fas fa-headphones"></i>'; // Reset icon
        button.disabled = false;
        // If we're resetting the *current* button, clear the global trackers
...
function handleTTSDelegatedClick(event) {
    ...
    if (!button || !synth) return; // Not a TTS button or synth not supported
    ...
    // Determine text source based on button
    if (isGlobalButton) {
        const articleBody = document.getElementById('article-body');
        // Prioritize textContent for cleaner text, fallback to innerText
    ...
    if (currentPlayingButton === button && currentUtterance) {
        // Clicked the button that is currently associated with speech
        console.log(`Clicked active button. Synth state: speaking=${synth.speaking}, paused=${synth.paused}, pending=${synth.pending}`);
        if (synth.paused) {
            console.log("Attempting to RESUME TTS.");
            synth.resume();
            button.innerHTML = '<i class="fas fa-pause"></i>'; // Show pause icon when resuming
        } else if (synth.speaking) {
            console.log("Attempting to PAUSE TTS.");
            synth.pause(); // Request pause
            button.innerHTML = '<i class="fas fa-play"></i>'; // Show play icon when paused
        } else {
            // Neither speaking nor paused, but button was marked as active? Reset state.
            console.log("Synth inactive but button was marked active. Resetting.");
            resetTTSButtonState(button);
            // Optionally try to replay? For now, just reset.
            // speakText(textToSpeak, button);
        }
    } else {
        // Clicked a new button, or the first time
        console.log("Clicked inactive/new button. Cancelling previous (if any) and starting new speech.");
        ...
        if (!textToSpeak) {
            ...
            // Button state already reset by cancelSpeech
            return;
        }
...
function speakText(text, button) {
    ...
    // Update button state immediately to 'loading'
    button.disabled = true;
    button.classList.remove('playing'); // Ensure playing class is off
    button.classList.add('loading');
    button.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';

    // Truncate long text for stability
    ...
    currentUtterance.onstart = () => {
        ...
        // Check if the button is still the intended target before updating state
        if (currentPlayingButton === button) {
            ...
            console.log("Speech started, but target button changed. Ignoring UI update for this button.");
        }
    };

    currentUtterance.onpause = () => {
        console.log("Browser speech paused.");
        // Update icon if the currently playing button is the one associated
        if (currentPlayingButton === button) {
            button.classList.remove('loading'); // Ensure loading is off
            button.classList.add('playing'); // Keep playing class? Or maybe a 'paused' class? For now, keep playing state but change icon.
    ...
    currentUtterance.onend = () => {
        ...
        // Only reset the button if it's the one that finished playing
        if (currentPlayingButton === button) {
            resetTTSButtonState(button);
        } else {
    ...
    currentUtterance.onerror = (e) => {
        ...
        // Always reset the button associated with the failed utterance
        resetTTSButtonState(button);
    };
