# src/agents/meta_description_agent.py
"""
Meta Description Agent for generating SEO-optimized and click-worthy meta descriptions.

This agent uses an LLM (currently configured for DeepSeek) to generate
meta descriptions based on article titles, keywords, and summaries.
It focuses on emulating top SERP snippet styles: direct, outcome-driven,
keyword-forward, and ruthlessly concise, with natural punctuation and
a zero-tolerance policy for common LLM clichés and hype.
"""
...
DEEPSEEK_API_KEY_META = os.getenv('DEEPSEEK_API_KEY')
DEEPSEEK_CHAT_API_URL_META = "https://api.deepseek.com/chat/completions"
DEEPSEEK_MODEL_FOR_META = "deepseek-chat" # A highly capable model is essential here

API_TIMEOUT_META_AGENT = 110 # Slightly more time for complex constraint adherence
...
def truncate_meta_description(text_str: str, max_length: int = META_DESC_HARD_MAX_LEN) -> str:
    ...
    text_str = text_str.replace('�', '—') # Handle potential Unicode issues
...
def call_deepseek_for_meta_description(h1_or_final_title: str,
                                       primary_keyword: str,
                                       secondary_keywords_list: list,
                                       processed_summary: str) -> str | None:
    if not DEEPSEEK_API_KEY_META:
        logger.error("DEEPSEEK_API_KEY_META not found.")
        return None
...
**Processed Summary**: {processed_summary_snippet}
    """
    payload = {
        "model": DEEPSEEK_MODEL_FOR_META,
        "messages": [
            {"role": "system", "content": META_AGENT_SYSTEM_PROMPT},
            {"role": "user", "content": user_input_content.strip()}
        ],
        "temperature": 0.82, # Increased for maximum punch and human-like variance
        "max_tokens": 300,
        "response_format": {"type": "json_object"}
    }
    headers = {"Authorization": f"Bearer {DEEPSEEK_API_KEY_META}", "Content-Type": "application/json"}

    try:
        logger.debug(f"Sending meta desc request for: '{title_context}'")
        response = requests.post(DEEPSEEK_CHAT_API_URL_META, headers=headers, json=payload, timeout=API_TIMEOUT_META_AGENT)
        ...
        if response_json.get("choices") and response_json["choices"][0].get("message") and response_json["choices"][0]["message"].get("content"):
            ...
            logger.info(f"DeepSeek meta desc gen successful for '{title_context}'.")
            logger.debug(f"Raw JSON for meta: {json_str}")
            return json_str
        logger.error(f"DeepSeek meta desc response missing content: {response_json}")
        return None
    except requests.exceptions.RequestException as e:
        logger.error(f"DeepSeek API req for meta desc failed: {e}. Response: {e.response.text[:500] if e.response else 'No response'}")
        return None
    except Exception as e:
        logger.exception(f"Unexpected error in call_deepseek_for_meta_description: {e}")
        return None
...
def run_meta_description_agent(article_pipeline_data: dict) -> dict:
    article_id = article_pipeline_data.get('id', 'unknown_id')
    logger.info(f"--- Running Meta Description Agent for Article ID: {article_id} ---")
    ...
    if not h1_or_final_title and not processed_summary:
        ...
        raw_llm_response = call_deepseek_for_meta_description(h1_or_final_title, primary_keyword, secondary_keywords, processed_summary)
    ...
    logger.info(f"Meta Desc Agent for {article_id} status: {article_pipeline_data['meta_agent_status']}.")
...
if __name__ == "__main__":
    logger.info("--- Starting Meta Description Agent Standalone Test (ASI-Level Prompt vFINAL) ---")
    if not DEEPSEEK_API_KEY_META: logger.error("DEEPSEEK_API_KEY not set. Test aborted."); sys.exit(1)

    sample_data = {
        'id': 'test_meta_asi_final_001',
        'generated_seo_h1': "NVIDIA Blackwell B200 Arrives, Crushes AI Speed Records", # More punchy H1 example
    ...
    result = run_meta_description_agent(sample_data.copy())
    logger.info("\n--- Test Results (ASI-Level vFINAL) ---")
    ...
    logger.info("\n--- Test Fallback (ASI-Level vFINAL) ---")
    minimal_data = {'id': 'test_fallback_meta_final_002', 'final_keywords': ["Tech Breakthroughs"]}
    result_min = run_meta_description_agent(minimal_data.copy())
