...
def run_image_integration_agent(article_pipeline_data):
    ...
    for candidate_info in media_candidates:
        ...
        vlm_caption = candidate_info.get('vlm_image_description', '') 
        ...
        safe_alt_text = alt_text.replace('"', "'").replace('[','(').replace(']',')')
        image_markdown = f"\n![{safe_alt_text}]({image_url})\n" 

        if vlm_caption and len(vlm_caption.strip()) > 10 and vlm_caption != "Image selected based on search query match." and vlm_caption != "Analysis N/A" and "Placeholder" not in vlm_caption :
        ...
        if placeholder_comment_to_find in modified_markdown_body:
            ...
            integration_log.append(log_msg)
            logger.warning(f"For {article_id}: {log_msg} - Candidate URL was {image_url}.")
...
if __name__ == "__main__":
    ...
    sample_article_data_for_integration = {
        ...
        'media_candidates_for_body': [
            ...
            },
            {
                ...
                'vlm_image_description': '' # Test no caption
            }
        ]
    }
    ...
    if "![Sleek new AI gadget](https://example.com/images/gadget_sleek.jpg)" in final_md and \
       "*Photo of the new AI-powered gadget in silver.*" in final_md:
        logger.info("SUCCESS: First image and its caption correctly integrated.")
    else:
        logger.error("ERROR: First image or caption not found/incorrect in final markdown.")
        
    if "![Gadget benchmark performance graph](https://example.com/images/gadget_benchmarks.png)" in final_md and \
       "*Bar chart comparing benchmark scores.*" not in final_md: # Assuming empty vlm_desc means no caption
        logger.info("SUCCESS: Second image correctly integrated (without caption as expected).")
    else:
        logger.error("ERROR: Second image not found or caption incorrectly added in final markdown.")
